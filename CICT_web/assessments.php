...
$assess_count = 1;
$_SESSION["assess_count"] = $assess_count;
    ...
    <nav class="navbar navbar-expand-md navbar-light bg-body border-bottom p-0 ps-5" id="navbarz">
        ...
        <div class="collapse navbar-collapse" id="navbarNavDropdown">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link border-bottom border-dark" href="assessments.php">Assessment<span class="sr-only">(current)</span></a>
                </li>
            </ul>
        </div>
                ...
                <div class="rounded overflow-auto w-100 text-start" style="height:750px;">
                    <div class="list-group list-group-flush mx-3 mt-4">
                        ...
                        <?php foreach($xml->assessment as $assessment) : 
                            ...
                            $locked = ($assessNo == $assesslid) ? false : true; // set locked state based on lesson ID and quiz count
                        ?>
                            <a href="#" class="list-group-item list-group-item-action py-2 ripple disabled text-dark mt-3" aria-current="true">
                                <h5 <?php if ($locked) echo "disabled"; ?>>Lesson <?php echo $assessNo?></h5>
                            </a>
                                <form method="post">
                                    <button type="submit" class="list-group-item list-group-item-action py-2 ripple border border-top-0 border-end-0 border-start-0 border-bottom-1 border-dark <?php echo $class ?>" value="<?php echo $assessNo?>" name="assess" <?php if ($locked) echo "disabled"; ?>>Assessment No. <?php echo $assessNo?></button>
                                </form>
                        <?php endforeach ?>
                    </div>
                </div>
                    ...
                    <div class="col py-2">
                        <div class="col text-end">
                            <div class="rounded overflow-auto w-100 text-start">
                                ...
                                <?php 
                                $takenAssess = array(); // an array to store the quizzes that the user has taken
                                ...
                                foreach($xmlUser->user as $user) {
                                    if ((string)$user['uid'] == $user_id) {
                                        ...
                                        foreach ($user->assess as $assess) {
                                            $assessNo = (string)$assess['aNo'];
                                            $assessScore = (string)$assess;
                                            $takenAssess[$assessNo] = $assessScore;
                                        }
                                        break;
                                    }
                                }

                                $canTakeAssess = !array_key_exists($selectedAssess, $takenAssess); // check if the selected quiz has not been taken yet
                                ?>

                                <p><?php 
                                    if (!isset($takenAssess[$selectedAssess]) || $takenAssess[$selectedAssess] == "") {
                                        echo "If you AGREE with the agreement, you can proceed to the Assessment.";
                                        $canTakeAssess = true;
                                    } else {
                                        echo "You have already taken Assessment $selectedAssess. Your score is: ";
                                        echo "<b>".$takenAssess[$selectedAssess]."</b>"; // display the score for the selected quiz
                                    }
                                    ?></p>

                                <?php if ($canTakeAssess) { ?>
                                    <div class="post-text pt-2 mx-2">
                                        <form method="post" class="mb-2">
                                            <input type="submit" class="btn btn-danger pull-right" name="take" value="Take Assessment <?php echo $selectedAssess?>" <?php echo $canTakeAssess ? '' : 'disabled' ?>>
                                        </form>
                                    </div>
                                <?php } ?>
                            </div>
