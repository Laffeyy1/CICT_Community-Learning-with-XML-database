...
if (isset($_POST['submit_d'])) {
    ...
    // Find the quiz with the given quizNo and the question with the given question number
    $questionToDelete = null;

    foreach ($xml->quiz as $quiz) {
        $quizNo = $quiz["quizNo"]; 
        if ($deleteQuiz == $quizNo) {
            foreach($quiz->item as $item){
                if($deleteQuestion == $item["itemNo"]){
                ...
                <div class="rounded overflow-auto w-100 text-start" style="height:750px;">
                    ...
                    ?>
                        <a href="#" class="list-group-item list-group-item-action py-2 ripple disabled text-dark mt-3" aria-current="true">
                            <h5>Lesson <?php echo $quizNo?></h5>
                        </a>
                            <form method="post">
                                <button type="submit" class="list-group-item list-group-item-action py-2 ripple border border-top-0 border-end-0 border-start-0 border-bottom-1 border-dark <?php echo $class ?>" value="<?php echo $quizNo?>" name="quiz">Quiz No. <?php echo $quizNo?></button>
                            </form>
                    <?php endforeach ?>
                    </div>
                </div>
        ...
        <div class="col">
            <div class="container mt-3">
                <h5>Manage Quizzes
                    ...
                    <button type="button" id="deleteBtn"class="btn border border-dark pull-right mx-3" onclick="location.href='a_edit_quiz.php'"<?php echo $disabled ?>>
                        Edit
                    </button>
                    <button type="button" id="deleteBtn"class="btn btn-success pull-right mx-3" onclick="location.href='a_create_quiz.php'">
                        Create
                    </button>
                </h5>
            </div>
                    ...
                    <div class="col py-2">
                        <div class="col text-end">
                            <div class="rounded overflow-auto w-100 text-start" style="height:700px;">
                                <?php foreach($xml->quiz as $quiz) :
                                    $quizNo = $quiz["quizNo"]; 
                                    if ($selectedQuiz == $quizNo) {// move this line outside the inner loop
                                ?>
